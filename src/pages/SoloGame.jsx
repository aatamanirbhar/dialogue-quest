...
export default function SoloGame() {
  ...
  async function loadQuotes() {
    ...
    if (!data || data.length === 0) {
      console.log("No quotes found for category:", normalizedCategory);
      setQuotes([]);
      setLoading(false);
      return;
    }
  ...
  const currentQuestion = quotes[currentIndex];
...
const correct = userAnswer === correctAnswer;
  ...
  function restartGame() {
    ...
    const reshuffled = [...quotes].sort(() => Math.random() - 0.5);
  ...
  if (!quotes.length) {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center gap-4 px-4 text-center">
        <h1 className="text-3xl font-bold">No Quotes Found</h1>

        <p>
          Your database has no quotes for category:
          <span className="text-yellow-400 ml-2">{category}</span>
        </p>
  ...
  return (
    <div className="min-h-screen bg-black text-white px-4 py-10 flex justify-center items-center">
      <div className="w-full max-w-3xl bg-zinc-900 rounded-3xl p-8 border border-zinc-700">
