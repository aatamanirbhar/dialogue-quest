...
const notifyAccountChange = (type = "account") => {
  window.dispatchEvent(new Event("dq-account-change"));
...
const getLocalHistoryKey = (userId) => `${LOCAL_HISTORY_KEY}:${userId}`;
...
const getHistoryKey = (entry) => {
  ...
  return String(metadata.history_key || entry?.history_key || "");
};

const getHistorySignature = (entry) => {
  const metadata = getHistoryMetadata(entry);
  const mixCategories = Array.isArray(metadata.mixCategories)
    ...
    ? [...metadata.players]
        .map(
          (player) =>
            `${String(player?.name || "")
              .trim()
              .toLowerCase()}:${Number(player?.score || 0)}`,
        )
        .sort()
        .join("|")
    : "";
...
const getHistoryTime = (entry) => {
  const time = new Date(entry?.created_at || 0).getTime();

  return Number.isNaN(time) ? 0 : time;
};

const isDuplicateHistoryEntry = (existing, next) => {
  ...
  const sameSignature =
    getHistorySignature(existing) === getHistorySignature(next);

  if (!sameSignature) return false;

  return (
    Math.abs(getHistoryTime(existing) - getHistoryTime(next)) <=
...
const readLocalHistory = (userId) => {
  ...
  try {
    const raw = window.localStorage.getItem(getLocalHistoryKey(userId));
...
const writeLocalHistory = (userId, entry, limit = 30) => {
...
const mergeHistory = (remoteHistory = [], localHistory = [], limit = 30) => {
  const merged = [];
  const sorted = [...remoteHistory, ...localHistory]
    .filter(Boolean)
    .sort((a, b) => {
      const timeDiff = getHistoryTime(b) - getHistoryTime(a);
  ...
  for (const item of sorted) {
    ...
    if (duplicateIndex >= 0) {
      ...
      if (existing.local_only && !item.local_only) {
...
const mapProfile = (profile, user) => ({
  ...
  plan: profile?.plan || (profile?.is_premium ? PLANS.PREMIUM : PLANS.FREE),
  roomsCreated: Number(profile?.rooms_created || 0),
...
async function getFreshAccount() {
  ...
  return ensureProfile(refreshedUser?.user || user);
}
...
export async function createAccount({ name, email, password }) {
  ...
  const { data, error } = await supabase.auth.signUp({
    ...
    password,
    options: {
      emailRedirectTo: `${window.location.origin}/multiplayer?auth=confirmed`,
      data: {
        name: cleanName,
      },
    },
...
export function onAccountChange(callback) {
  ...
  const handleStorage = (event) => {
    if (active && event.key === ACCOUNT_SYNC_KEY) {
      refresh();
    }
  };

  window.addEventListener("dq-account-change", handleLocalChange);
  ...
  return () => {
    active = false;
    window.removeEventListener("dq-account-change", handleLocalChange);
...
export async function recordPlayHistory(entry) {
  ...
  const entryMetadata = getHistoryMetadata(entry);
  const payload = {
    user_id: user.id,
    mode: entry.mode,
    ...
    opponent_count: Number(entry.opponentCount || 0),
  ...
  const localEntry = writeLocalHistory(user.id, payload)?.[0];
  ...
  if (error && /relation|table|schema cache/i.test(error.message || "")) {
    response = await writeHistory(LEGACY_HISTORY_TABLE);
    data = response.data;
...
export async function getPlayHistory(limit = 30) {
  ...
  let response = await readHistory(HISTORY_TABLE);
  let { data, error } = response;

  if (error && /relation|table|schema cache/i.test(error.message || "")) {
    response = await readHistory(LEGACY_HISTORY_TABLE);
    data = response.data;
    error = response.error;
  }
  ...
  return mergeHistory(data || [], localHistory, limit);
}
...
export async function useTrialCredit(account) {
  ...
  const nextRoomsCreated = Number(account.roomsCreated || 0) + 1;
